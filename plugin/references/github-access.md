...
## ヘルパースクリプトが失敗したときの扱い（`gh` → MCP）

`fetch-*.sh` / `resolve-pr-comments.sh` のようなヘルパースクリプトは、中で `gh` を呼ぶ**「`gh` 経路」そのもの**である。したがって上の判定手順がそのまま適用される: スクリプトが認証エラー（401 / 403）で落ちたら、それは `gh` 経路が使えないという意味なので、**同じスクリプトを再試行せず、GitHub MCP で同じデータを取り直す**。

クラウドセッションではこれが常態になる。`gh api graphql` / `gh pr list` / `gh issue list` は GraphQL 経由なのでプロキシで 403 になり、スクリプトは必ず落ちる。スクリプトの失敗をもって収集を諦めると、そのタスクは何も見ずに終了する。

- **スクリプトの失敗を「0件」に読み替えない**（下の「`gh` 失敗の扱い（収集系スクリプト）」参照）。失敗は失敗として扱い、MCP へ切り替える
- **MCP 経路の出力は、スクリプトの出力と同じ構造（キー名・値の意味）で組み立てる。** 後続フェーズは経路を区別せず同じ `jq` コマンドでこれを読むため、キーが揃っていないとそこで壊れる。スクリプトが `output_file` にJSONを書き出す形式の場合は、MCP 経路でも同じ構造のJSONを `Write` で書き出し、そのパスを `output_file` として扱う。必要なキーは各スキル本文の「MCP経路の（`output_file`）契約」「MCP経路の収集契約」に定義してある
- **MCP でも取得できなかった場合は、そこで初めて失敗として終了する。** 空の結果を返してはならない（呼び出し元が「指摘なし」「該当0件」と誤認して、未対応の指摘を残したままPRをマージしたり、その期間の収集をスキップしたりする）
