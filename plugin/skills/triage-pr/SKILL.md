...
- ステップ3のアクション: 修正が必要なら`cc-fix-onetime`ラベル付与 / マージ可能なら、Epic PR（`cc-epic-issue`付き）は`cc-release-ready`ラベル付与（マージしない）、通常PRはマージ
...
`cc-ui-design` ラベル付きPRで、CI失敗の原因がベースブランチ側の障害や実装コードに対する必須チェック（`.pen`/スナップショットPNGの変更では直接修正できないもの）である場合、`cc-need-human-check`ラベルを追加し終了する。`cc-fix-onetime`は付与しない（コード修正でしか直らない失敗を`.pen`変更で解消しようとするtriage-pr⇔fix-review-pointの無限ループを避けるため）。
