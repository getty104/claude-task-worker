...
メインエージェント（本スキルを実行しているセッション本体）の役割は「分解・委譲・検証・統合」であり、実装の主体はサブエージェントである。ただし、1ファイル数行の修正まで委譲するとブリーフィング作成とサブエージェント側の再探索のほうが高くつくため、**下記の判定に従って委譲/直接編集を決める**。

判定は**着手前**に行い、判定結果（どちらを選んだか・根拠となった条件）を記録して最終報告の自己監査に載せる。

### ステップ1: 無条件で委譲するもの（1つでも該当したら即委譲。以降の判定はしない）

- フェーズ3で実行する**実装タスク本体**（フェーズ1で分解されたタスク）。規模の大小を問わず必ず委譲する。メインの直接編集が許されるのはフェーズ5以降の収束作業に限る
...
1. フェーズ5の収束ループ（テスト/Lint/ビルド失敗の修正）、またはサブエージェント成果物の範囲外変更の巻き戻しである
2. 対象が単一ファイルで、変更が概ね10行以内
3. 対象箇所が**すでにメインの文脈にある**（自分で `Read` 済み、CodeGraph の出力に含まれる、または失敗ログが `file:line` を直接指している）
4. 追加の調査・仕様確認が不要で、修正内容が一意に定まる（型注釈の修正、import の追加、明らかなtypo、スナップショット更新など）
...
小修正の直接編集を積み上げて実装そのものを自分でやってしまう事故を防ぐため、以下を守る。

- 直接編集は**1回ごとに独立して判定**する。「さっき直したついでに」で連鎖させない
- 直接編集したら**即座にテスト/Lintを再実行**して結果を確認する（まとめて後で確認しない）
- **同一の収束ループ内で直接編集が3回に達したら、以降はすべて委譲へ切り替える**（3回直しても収束しない＝ステップ2の条件4「修正内容が一意に定まる」が成り立っていない）
...
通常タスク（スキル実行ステップ以外）は、以下の判断軸でサブエージェントを選定し、実行を委任する。フェーズ3で実行する実装タスク本体は「実装の委任（判断ロジック）」のステップ1により**規模を問わず全件が委譲対象**なので、**すべての通常タスクがいずれかのサブエージェントに割り当たっている状態にする**（このフェーズに「自分で実装する」という選択肢は無い）：
...
- **実装はサブエージェントへ委譲する**: フェーズ3の実装タスク本体は規模を問わず全件委譲する。メインの直接編集はフェーズ5の収束ループで「実装の委任（判断ロジック）」ステップ2の5条件をすべて満たす場合に限り、滑り坂ガード（同一ループ内3回で以降は委譲へ切替）付きで許される。判定が割れたら委譲側に倒す
