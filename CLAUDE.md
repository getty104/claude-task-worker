...
両スキルはタスクをサブエージェントへ委譲する設計だが、選定基準（フェーズ2）とブリーフィング（フェーズ3）を書くだけでは**メインエージェントが自分で実装してしまう**（特に小粒な修正・テスト/Lintの収束ループでの修正）。委譲のオーバーヘッドを避ける方向にモデルが倒れるため、両スキル本文に「実装の委任（絶対規則）」セクションを置き、以下を規定している:

- メインエージェントに許可される操作の**ホワイトリスト**（読み取り・`git`/`gh` の参照・テスト/Lintの実行・`Skill` 発火・`Agent` 起動・スキルが指示する Issue/PR 操作）と、禁止される操作（`Edit`/`Write`/`NotebookEdit`、リポジトリ内ファイルを書き換える `Bash`）を明示する
- **規模による例外を認めない**（1行・typo・import追加・自明な型エラーも委譲。軽量タスクは `lightweight-assistant` が受け皿）
- テスト/Lint収束ループの修正も委譲対象であることを、該当ステップ内で再掲する（ここが最も自分で直しやすい箇所のため）
- 最終報告に「実装委任の自己監査」（起動したサブエージェントの内訳・メインによる直接編集の有無）を必須項目として出力させる
