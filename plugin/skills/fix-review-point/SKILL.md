...
本スキルは `worker-skill-executor` エージェント（`plugin/agents/worker-skill-executor.md`）上で `context: fork` 実行される。バックグラウンド実行の禁止・同期実行の徹底・TaskCreate の作法・自律実行原則といった共通ルールはエージェント定義に集約されており、必ずそれに従うこと。特に `create-review-fix-plan` / `commit-push` / `resolve-pr-comments` の各スキルは、返却された修正プラン・push完了・resolve完了を同期的に受け取ってから次のフェーズに進む。
