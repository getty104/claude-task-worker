...
既存Issueの title/body・`docs/`・README・`.claude/requirements/`・コード内コメント・設定ファイルには、判断の根拠が**リポジトリ外**にあるリンク（仕様書・設計ドキュメント・API仕様・ライブラリ公式ドキュメント・関連する別リポジトリのIssue/PR・デザインURLなど）が置かれていることがある。`gh` とローカルファイルだけで調べて「不明」「確認事項」に倒すと、リンク先に答えが書いてあるのに人へ差し戻すことになる。**リンク先の内容まで読んで結論を確定させる**。
