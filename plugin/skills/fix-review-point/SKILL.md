...
メインエージェント（本スキルを実行しているセッション本体）の役割は「指摘の分解・委譲・検証・統合」であり、修正の主体はサブエージェントである。ただし、レビュー指摘には1ファイル数行で済むものが相当数あり、そこまで一律に委譲するとブリーフィング作成とサブエージェント側の再探索のほうが高くつく。**指摘ごとに下記の判定を行って委譲/直接編集を決める**（プラン全体で一括判定しない）。
...
上記のどちらとも言い切れない場合は**委譲側に倒す**。「たぶん小さい」「たぶん1ファイルで済む」は該当しない根拠にならない。軽量な指摘の受け皿として `lightweight-assistant` を使う。
...
修正タスク（指摘）ごとに、まず「実装の委任（判断ロジック）」のステップ1〜3で**委譲するか直接編集するか**を判定する。委譲すると決めたタスクについて、以下の判断軸でサブエージェントを選定し、実行をそのサブエージェントに委任すること
