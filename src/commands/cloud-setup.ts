...
// `Bash(bash *)` は `bash ${CLAUDE_PLUGIN_ROOT}/scripts/gh-compat.sh ...` の形式で呼ぶ
// Issue/PR の作成・ラベル/Assignee 付与を許可対象に含めるため（`Bash(gh *)` にはマッチしない）。
// 既存の allow は残し、欠けているルールだけを足す（配列の和集合なので force の対象外）。
export const CLOUD_PERMISSION_ALLOW = ["Bash(gh *)", "Bash(git *)", "Bash(bash *)"] as const;
