...
- **`src/commands/init.ts`** - GitHub ラベル初期作成コマンド。あわせて Issue テンプレート・GitHub Actions ワークフロー・設定ファイル（`claude-task-worker.json`）の生成、CodeGraph のセットアップ（グローバル gitignore への `.codegraph/` 登録 → `codegraph init` によるインデックス構築）、`.claude/settings.json` への設定書き込み（`permissions.defaultMode` = `"auto"`、`outputStyle` = `"Proactive"`、`language` = `"Japanese"`）を行う
  - `permissions.defaultMode` はクラウド実行（`--cloud`）の権限モードを決めるためだけに置く。クラウドセッションは `--permission-mode` を**受理するが VM 側に反映しない**（`--disallowedTools` / `--append-system-prompt-file` と同じ。Issue #307）うえ、`bypassPermissions` がクラウドのモード一覧に無いため既定の `acceptEdits`（「編集を受け入れる」）へ落ちる。公式が挙げるクラウドセッションの設定手段は「環境変数」と「リポジトリにコミットした設定ファイル」の2つで、権限モードを指す環境変数は存在しない（`CLAUDE_CODE_*` に該当なし）ため、コミットする settings ファイルが唯一の経路になる。ローカル実行（default / herdr）はワーカーが `--permission-mode` フラグを渡し、フラグが settings に勝つため挙動は変わらない
  - `outputStyle` / `language`（`CLAUDE_SETTINGS_DEFAULTS`）も同じファイルへ書く。タスクセッションは応答するユーザーが常駐しない自律実行なので確認を挟まず進む `Proactive` を、成果物（Issueコメント・PR本文・最終報告）の言語を揃えるため `Japanese` を既定にする。ユーザーの `~/.claude/settings.json` に同じ設定があってもクラウドセッションには渡らない（ユーザー側の設定はマシンに閉じ、リポジトリにコミットしたものだけが届く）
  - 書き込みは `withInitDefaults()`（純粋関数）による**マージ**で行い、既存の `.claude/settings.json` を丸ごと上書きしない（対象リポジトリには hooks・`enabledPlugins`・`permissions.allow` が同居しているのが普通で、上書きすると init が破壊的操作になる）。既に指定があるキーは人の選択を尊重して触らず（キー単位の判定なので、欠けているキーだけが埋まる）、`--force` のときだけ上書きする。JSON として読めない・`permissions` がオブジェクトでない場合は書き換えずスキップしてログに出す
