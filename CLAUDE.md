...
- **`src/commands/init.ts`** - GitHub ラベル初期作成コマンド。あわせて CodeGraph のセットアップ（グローバル gitignore への `.codegraph/` 登録 → `codegraph init` によるインデックス構築）と、プロジェクト設定（`.claude/settings.json`）への本プラグインの登録も行う
