...
- テンプレート外の見出し（`##`）を追加していない（末尾の変更ログ折りたたみは見出しではないため対象外。依存関係も `## 依存関係` として本文に書かず、`blocked_by` / `blocking`（GitHub relationships）で表現する）。ただし `mode=edit` で既存bodyにあった `## UIデザイン` は上記の verbatim 保持ルールに従って残す
- `mode=edit` で既存bodyに `## UIデザイン` があった場合、その中身が1文字も変わらずに再掲されている
