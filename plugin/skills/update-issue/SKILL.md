...
- コメント同士、またはコメントと既存本文で結論が食い違い、どちらを採るか確定できない
...
- **ステップ2.5で「既存bodyに依頼内容あり」と判定した場合** — 以下の手順で検証する:
  ...
  5. 一致しない、または折りたたみになっていない場合は次の順で復旧を試みる:
     - argsファイル（ステップ5と同じパス・同じ YAML）を書き直したうえで、もう一度 `post-issue-body` を同じ args で呼び直す（一過性の投稿エラーの可能性。argsファイルは前回呼び出しで消費済みのため再書き込みが必須）。
