...
出力したTODOリストは後段（`exec-issue` 等）がそのまま実装スコープとして読む。**依頼にある要求を構造化する**のが責務であり、要求を増やす場ではない。

- 依頼に含まれない作業（周辺のリファクタ、テスト基盤の新規導入、命名統一、パフォーマンス改善など）をTODOに足さない。必要だと考える場合は「スコープ外」または「仮定事項」に1行で挙げるだけにする
- 依頼の前提が間違っていると考えた場合は「仮定事項」に1-2行で書き、そのうえで**依頼どおりのスコープで**分解を完遂する
- TODOの件数は依頼の規模に見合わせる。粒度を細かくするために1タスクを機械的に分割しない

## 出力の分量

- 各TODOの「説明」は1〜2行。要件の再掲や同じ内容の言い換えを書かない
- 「非機能要件」「スコープ外」「前提条件」「仮定事項」は該当があるものだけ書く（無い節は「なし」の1語で済ませ、埋め草の項目を作らない）
- 依存関係図・クリティカルパスは、依存が2段以上あるときだけ出す

## 品質基準

- 各TODOは1人が1回の作業セッションで完了できる粒度にする
- 依存関係に循環を作らない（`T-1 → T-2 → T-1` のような相互依存は分割し直す）
- 曖昧な表現を避け、完了条件が明確なタスクにする
- 要件からTODOへのトレーサビリティを保ち、どの要件にも紐づかないTODOを作らない
