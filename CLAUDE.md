...
- **`src/commands/cloud-setup.ts`** - クラウド実行（`--cloud`）用の VM 側セットアップコマンド。claude.ai の環境設定のセットアップスクリプト欄から `npx claude-task-worker cloud-setup` として呼ぶ想定で、**VM 側でしか意味を持たない準備をここへ集約する**（settings ファイルの書き込み、CodeGraph のセットアップ）。`cloudSetup()`（各ステップの呼び出し）、`withCloudDefaults()` / `claudeSettingsPath()`（テスト可能な純粋関数）
  ...
  - **CodeGraph のセットアップ**は `init` と同じ2つ（`ensureCodegraphGitIgnore()` → `runCodegraphInit()`）。`codegraph init` はカレントディレクトリのリポジトリを対象にするため、`.git` が無い場合は実行せずスキップする（クローンより前にセットアップスクリプトが走る場合。スキップしてもワーカーは動き、探索がテキスト検索に落ちるだけ）。**CLI 本体（`npm install -g`）はここでは入れない** — `install` が同じことをするので、セットアップスクリプトに両方書けば二重に走るだけになる。未導入なら `runCodegraphInit()` が `claude-task-worker install` を促すログを出して false を返す
  - **環境キャッシュとの相性には制約がある**。セットアップスクリプトが走るのは**キャッシュが無いときだけ**（既存スナップショットがあるセッションではスキップされる）で、キャッシュはリポジトリを問わず再利用される。つまり `codegraph init` で作った `.codegraph/` が毎セッション最新である保証は無く、別リポジトリのセッションには効かない。インデックスが古い/無い場合の扱い（テキスト検索へフォールバック）は元から想定済みなので実害は無いが、「セットアップスクリプトに書けば必ずインデックスが載る」とは考えないこと
