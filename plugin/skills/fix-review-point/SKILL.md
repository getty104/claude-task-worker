...
- UIデザインのマークアップ（見た目・スタイル・デザインとの一致に関する指摘） → `frontend-implementer`（マークアップ以外のフロントエンド指摘＝状態管理・データ取得・API連携・ロジックは同エージェントの担当外。`general-purpose-assistant` へ委譲する）
...
`gh pr checks $0` でCIの全チェックが Pass していることを確認する。Pass している場合は `gh pr merge $0 --merge --delete-branch` でPRをマージし、下記「マージ後の関連Issue連動Close」を実行して終了する。チェックが失敗中の場合はマージせず、CI失敗状況をレビュアーに報告して終了する。
...
- **frontend-implementer**: **UIデザインのマークアップ専任**。デザインとの視覚的な差異・スタイル・マークアップ構造・a11yマークアップに関する指摘にのみ使う。`.pen` を**参照元**としてUIに変換する指摘は**必ず**このエージェントを使う。逆に、状態管理・データ取得・API連携・バリデーション/ビジネスロジックへの指摘はフロントエンドのファイルが対象でも**このエージェントに渡さない**
...
マークアップ修正が数行で完結し、分割コストが上回る場合は分割せず1タスクにまとめてよい。その場合の担当は `general-purpose-assistant` にする。
