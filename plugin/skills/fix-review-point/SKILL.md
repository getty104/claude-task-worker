...
**メインエージェント（本スキルを実行しているセッション本体）はコードを自分で書かない。** リポジトリ内ファイルの作成・編集・削除は、例外なくサブエージェント（フェーズ2の選定基準に従う）に委譲する。メインエージェントの役割は「指摘の分解・委譲・検証・統合」であって実装ではない。
...
- `Skill` ツールによるスキル発火（`create-review-fix-plan` / `commit-push` / `resolve-pr-comments` など）
- `Agent` ツールによるサブエージェント起動
- スキル本文が明示的に指示する Issue/PR 操作（`gh pr edit` / `gh pr merge` / `gh pr close` / `gh issue comment` など）
...
**規模を理由にした例外は認めない。** 「1行だけ」「typoの修正」「変数名の変更」「コメント削除」「自明な型エラーの修正」といったレビュー指摘も委譲する。レビュー指摘は小粒なものが多いが、それこそが本スキルでメインエージェントが自分で直してしまう主因であり、明示的に禁止する。軽量な指摘は `lightweight-assistant` に渡す。
...
タスク毎に以下の判断軸でサブエージェントを選定し、タスクの実行はそのサブエージェントに委任すること。**すべての修正タスクがいずれかのサブエージェントに割り当たっている状態にする**（「これは自分でやる」という選択肢は無い。「実装の委任（絶対規則）」参照）
...
3. 最終報告として、対応した指摘の件数とPRのURLを出力する。あわせて以下のフォーマットで自己監査結果を必ず含める（「実装の委任（絶対規則）」の遵守状況をセッション自身で振り返るためのもので、違反の隠蔽より事実の記録を優先する）
   ```markdown
   ## 実装委任の自己監査
...
- **修正は必ずサブエージェントへ委譲する**: メインエージェントは `Edit`/`Write`/`NotebookEdit` およびファイルを書き換える `Bash` を使わない。1行の指摘・テスト修正・Lint修正も例外なく委譲する（詳細は「実装の委任（絶対規則）」）
