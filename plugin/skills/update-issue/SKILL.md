...
- **既存bodyに旧フォーマットの `## 依頼内容` 見出しがある場合**（本フォーマット移行前の Issue）
  - 同じく本スキルからは `依頼内容` を渡さない（`post-issue-body` 側が旧フォーマットを検出して折りたたみブロックへ詰め替える）。ステップ5.5の期待値は、見出し直後から次の `## ` 見出しまたはEOF直前までの中身を切り出して保持する。
- **どちらも無い場合** — 何もしない。依頼内容ブロックは無いままにする（更新後 description は従来通り `## 概要` から始まる6セクション構成になる）。

**完了条件**: 期待値の「依頼内容」文字列（存在する場合はその中身、無い場合は「なし」フラグ）と、期待される書き出しフォーマット（常に `<details><summary>依頼内容</summary>` の折りたたみ）が確定していること。
