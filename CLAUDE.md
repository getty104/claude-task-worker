...
- **`src/commands/init.ts`** - GitHub ラベル初期作成コマンド。あわせて CodeGraph のセットアップ（グローバル gitignore への `.codegraph/` 登録 → `codegraph init` によるインデックス構築）と、`ctw-solve-github-workflow-problem` スキルの `.claude/skills/` へのコピーも行う
  - スキル本体は `plugin/skills/ctw-solve-github-workflow-problem/SKILL.md` が唯一の正で、init.ts は **esbuild の `--loader:.md=text`** でその中身を文字列として import する（型は `src/skill-md.d.ts` の `declare module "*.md"`）。npm パッケージは `files: ["dist"]` で `plugin/` を同梱しないため実行時にファイルを読めず、かといって内容を init.ts へ写経すると plugin 側と二重管理になる。バンドル時に取り込むことで両方を避けている
- **`src/commands/install.ts`** - マーケットプレイス追加・プラグインインストール・CLI自体のインストール・CodeGraph CLI / DESIGN.md CLI / Pen CLI のインストールを一括で行うコマンド
- **`src/commands/update.ts`** - プラグイン/マーケットプレイス・CLI自体・CodeGraph CLI / DESIGN.md CLI / Pen CLI の更新コマンド
...
- **`src/commands/pen.ts`** - Pen CLI（[`@pen.dev/cli`](https://pen.dev)）連携。`installPenCli()` の1関数のみで、`install` / `update` の**どちらからも同じ関数を呼ぶ**（DESIGN.md CLI と同じく self-upgrade 機構を持たないため、更新手段が `npm install -g <pkg>@latest` しかない）。**新パッケージの導入前に旧 `@pencil.dev/cli` を必ず `npm uninstall -g` する**のが本関数の要点で、両者は同じ bin 名 `pencil` を提供するため同居させると解決先が不定になる。旧 0.2.x を掴むとツール構成が違う（`batch_design` 等の廃止済みツール）ため `edit-pencil-design` / `inspect-pencil-node` / `resolve-pencil-conflict` の各スキルが動かない。削除 → 導入の順にすることで `pencil` が確実に新パッケージへリンクされる。旧パッケージ未インストール時の `npm uninstall` 失敗は「削除の目的が既に達成されている」ためログのみで続行する
- **`src/commands/pencil.ts`** / **`src/pen-baseuri-fix.ts`** - `claude-task-worker pencil <args...>` ラッパー。`@pen.dev/cli` はアセットのベース URI を相対パスのまま返すため `.pen` を開くと `Base URI must be absolute!` で失敗する。`src/pen-baseuri-fix.ts` は Node の `registerHooks()` でその getter を**ロード時にメモリ上だけ**書き換える loader hook で、`pencil()` が `NODE_OPTIONS="--import <hook の file:// URL>"` を注入して `pencil` を spawn する（`stdio: "inherit"` で引数・stdin・終了コードをそのまま素通しするため、heredoc を使うスキルの記述が `pencil` を `claude-task-worker pencil` に置き換えるだけで済む）
  - **hook はバンドルとは別ファイルとして `dist/pen-baseuri-fix.mjs` へ出す**（`package.json` の `build` が esbuild を2回呼ぶ）。`--import` は実ファイルの URL を要求するため `dist/index.js` へインライン化できない。パスは `import.meta.url` からの相対で解決する（`version.ts` の `package.json` 解決と同じ方式）
  - **NODE_OPTIONS へは `pathToFileURL()` の `file://` URL を渡す**。同変数は空白区切りで値中の空白をエスケープできないが、file URL は空白を `%20` へエンコードするため、インストール先に空白を含む環境でも壊れない。既存の `NODE_OPTIONS` は後ろに連結して保持する（`buildPencilNodeOptions()`）
  - hook は難読化されたコードを正規表現で書き換えるため、`@pen.dev/cli` の更新でパターンが変わると**黙って no-op になる**（マッチしなければ元のソースをそのまま返す）。`.pen` のアセット読み込みが再び失敗し始めたらこの正規表現を疑う
  - `pencil` はワーカーではないため `--project` と併用できない（`dispatch-args.ts` の `PROJECT_INCOMPATIBLE_COMMANDS`）
...
- Pen CLI (`pencil` / `pen`) がインストール済み（`claude-task-worker install` / `update` が面倒を見る）
  ...
  - **呼び出しは常に `claude-task-worker pencil <args...>` 経由**（素の `pencil` を直接呼ぶとアセット URI のバグを踏む）。上記の `src/commands/pencil.ts` 参照
