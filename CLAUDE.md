...
- **`src/herdr.ts`** - herdr CLIラッパー。`workspaceCreate`/`workspaceList`/`workspaceClose`/`workspaceFocus`（ワークスペース管理。`workspaceList` の `focused` はフォーカス復元の判定に使う）、`tabCreate`/`tabRename`/`tabClose`/`tabList`（タブ管理）、`launchAgentInPane`（既存ペインのシェルへ起動コマンドを流し込んで claude(TUI) を起動。`shellQuoteArgv` で argv をシングルクォート quoting した1行を `paneSendText` → `paneSendKeys enter` で送る）、`shellQuoteArgv`（argv → シェル安全な1行。テスト可能な純粋関数）、`agentGet`（agentステータス取得。`agent_session.kind === "id"` のときは claude のセッションIDも返す）、`paneSendText`/`paneSendKeys`（ペインへの入力送信）、`paneRead`（ペインの端末内容取得）、`paneGet`/`paneClose`、`paneProcessInfo`（フォアグラウンドプロセス確認）、`getCurrentWorkspaceId`（herdrが各ペインへ自動注入する `HERDR_WORKSPACE_ID` の読み出し）、`checkHerdrAvailable`（herdr導入・疎通確認）
  - **`herdr agent start` は使わない**。herdr 0.7 系で `agent start` の API が変わり、`--workspace`/`--tab`/`--cwd`/`--env` が廃止され `agent start <name> --kind <kind> --pane <id> [-- <args>]` になった。これは **`--kind` の正規実行ファイル（claude なら PATH 上の `claude`）を起動する**ため、`headroom wrap claude ...` のようなラッパー経由の起動ができない（`config.json` の `headroom: true` が使えない）。加えて旧引数を渡すと `unknown option: --workspace` で失敗する。代わりにタスクタブのルートペイン（シェル）へ起動コマンドを send-text で流し込む方式にした。claude は herdr の**自動エージェント検出**でそのまま agent として捕捉されるため、`agentGet(paneId)` で状態・セッションIDを取得できる（`agent start` による明示登録は不要）。cwd と env は `tabCreate` の `--cwd` / `--env` でシェルへ渡し、そこから起動する claude が継承する
...
- **`src/herdr-runner.ts`** - herdrモードのタスク実行。`startHerdrTask()`（`tabCreate`（`--no-focus`）→ `waitForPaneReady`（シェルプロンプト描画待ち）→ `launchAgentInPane`（ルートペインへ起動コマンド送信）→ `waitForAgentDetected`（claude が agent 検出されるまで待つ。旧 `agent start` の「起動確認」の代替で、検出できなければ失敗確定してタブを閉じる）。ルートペインがそのまま claude のペインになるため余剰シェルペインの `paneClose` は不要）、`waitForHerdrTask()`（agentステータスのポーリング。`done` または `working`→`idle` で完了、`pane_not_found`/`agent_not_found` で失敗、`blocked` は待機継続）、`buildHerdrTaskResult()`（ペイン出力が空なら空振りとして失敗扱い）、`stopHerdrTask()`（ctrl-c送信 → `waitForAgentGone` → タブクローズ）、`taskTabLabel()`（`ctw:<project>:#<n>`）
...
1. `tabCreate` で `ctw:<project>:#<番号>` ラベルのタスク専用タブを `--no-focus` で作り（ユーザーが見ているタブに割り込ませないため）、その**ルートペイン（シェル）へ `launchAgentInPane` で起動コマンドを send-text し** claude(TUI) を起動する。旧実装は `herdr agent start ... --tab <タブ> -- claude <引数>` で split 起動していたが、herdr 0.7 系で `agent start` が `--kind` の正規実行ファイルしか起動できなくなり（`--workspace`/`--tab` 廃止・`headroom wrap` 不可）、send-text 方式へ切り替えた（`src/herdr.ts` の `launchAgentInPane` 参照）。ルートペインがそのまま claude のペインになるため、旧実装の「split で余ったシェルペインを `paneClose` する」処理は不要。`waitForPaneReady`（シェル初期化中に送ると入力が捨てられるレース対策）→ send-text → `waitForAgentDetected`（claude が agent 検出されるまで待ち、起動失敗を早期に失敗確定してタブを閉じる。検出前の `agent_not_found` を後段の `waitForHerdrTask` が握ると無限待ちになるため）の順。ワークスペースは herdr が注入する `HERDR_WORKSPACE_ID` から解決するため、`--project` 経由ならそのプロジェクトのワークスペース内に作られる。プロジェクト名は `CTW_PROJECT_NAME`（ディスパッチャーが注入）→ `config.json` の逆引き → cwd のディレクトリ名の順で解決する
...
コマンドの組み立ては `src/claude-args.ts` の `buildClaudeExecution()`（`{ command, args }` を返す）。`mode` とは直交し、default モードでは `spawn` の command（シェル非経由、クォート不要）に、herdr モードでは `launchAgentInPane` が組み立てる `[command, ...args]` の argv になる。herdr モードは send-text でシェルへ流し込むため、`shellQuoteArgv` が各トークンをシングルクォート quoting してシェルの解釈を無効化する（SYSTEM_PROMPT のようなバッククォート・`$`・改行を含む引数もそのまま claude へ渡る）。
...
- **`HEADROOM_LOSSLESS=1` を必ず渡して CCR の retrieve ツール注入を無効化する**（`buildClaudeEnv()` が `headroom` 有効時に注入）。これは `API Error: 400 Tool reference 'headroom_retrieve' not found in available tools` の根本対策。原因は **CodeGraph MCP の出力と headroom の CCR マーカー正規表現の衝突**: CodeGraph はページング用に `[N items compressed to M. Retrieve more: hash=<24hex>]` を出力へ埋め込むが、これが headroom の CCR マーカー検出正規表現（`ccr/tool_injection.py`）にバイト単位で一致する。ワーカーは探索に CodeGraph MCP を使うため、その出力を headroom プロキシが「自分が圧縮した内容」と誤検知して `headroom_retrieve` ツールを request の `tools` に注入し、モデルが CodeGraph 出力中の "Retrieve more: hash=..." に釣られて同ツールを呼ぶ（headroom の圧縮ストアにその hash は無く空振りする）。この tool_use は会話履歴に残るが、次ターンでは CodeGraph のマーカーが frozen prefix 側に入り「新規マーカー無し」と判定され prompt cache 保護のため retrieve ツールが再注入されず、履歴の tool_use がツール定義を失って 400 になる。`HEADROOM_LOSSLESS=1` は headroom プロキシ起動時に `ccr_inject_tool=False` へ落とすため（`proxy/server.py`）検知しても注入せず連鎖を断つ。`HEADROOM_NO_CCR=1` と違い圧縮自体は lossless（format-native compaction + marker-free SmartCrusher）で維持されツール出力の情報欠落は起きない。**注意: 同 env は headroom の hot-reload 対象 knob ではなくプロキシ起動時のみ反映されるため、既に常駐しているプロキシは `headroom wrap` から再利用されて env を拾わない。設定反映にはプロキシの再起動が一度必要**
