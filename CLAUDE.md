...
- 経路は `triage-created-issue` のパターンE-1（パターンD通過後・パターンEの手前）で分岐する。**`.pen` の新規作成・編集はこのデザイン先行フロー専任**であり、UI変更タスクは前提ゲート（E-1-b）を満たす限り `cc-exec-issue` を付けず `cc-create-ui-design` のみを付与して終了する。**実装PR（`exec-issue`）で `.pen` を編集する経路は存在しない**（かつては description 末尾へ `## デザインファイルの更新` セクションを追記して実装と同じPRで `.pen` を更新させる「経路2」があったが撤去した。デザインの合意プロセスを迂回してデザインファイルが実装ついでに書き換わるのを防ぐため）。判定が割れたら**UI変更である側**へ倒す。デザイン検討の余地が無い場合や既存デザインの微修正でも例外を作らない（他に `.pen` の編集経路が無いため、例外はそのままデザイン未更新を意味する）
- 前提ゲートを満たさない場合の扱い: 条件2/3（`cc-ui-design-*` ラベル済み・description に `## UIデザイン` の実パス行あり）はデザイン合意済みとしてそのままパターンEへ。条件1（`uiDesign.enabled !== true`）はデザイン先行フローのワーカーが起動しないためパターンEへ進み、`git ls-files '*.pen'` が1件以上ならIssueへ「デザインファイルは更新されない」旨をコメントする（実装PRで `.pen` を編集させる指示は書かない）
