...
| 3 | リポジトリへのプラグイン宣言 | ~~可能（本Issueのスコープ外。#239 が担当）~~ **撤去した**（Issue #268）。クラウドセッションが `.claude/settings.json` の宣言を読んで自動的にプラグインを有効化するという前提が事実でなかったため、静的検査ごと撤去。代わりにクラウド VM のセットアップスクリプト（`scripts/cloud-setup.sh`）で `npx claude-task-worker install` を実行する方式へ置き換え、静的検査は行わない | 案内のみ（静的検査なし） |
