...
本スキルは `claude-task-worker` の `cc-exec-issue` ラベルをトリガーに自動起動される想定で、ワーカーはスキルプロセスの同期完了を根拠に `cc-in-progress` の除去や `cc-pr-created` 付与といったラベル遷移を進める（ただしフェーズ7で `cc-need-human-check` を付与した場合、ワーカーは PR 不在とみなして `cc-pr-created` を付与しない）。そのため、**本スキル内部で呼び出す `Agent` / `Skill` / `Bash` を絶対にバックグラウンド実行しないこと**：
...
- **「完了通知を待ちます」のような通知待ち状態でターンを終えない**。本スキルは print モードで動いており、ターンを終えた時点でプロセスが正常終了し、ワーカーが「完了」と誤認してラベル遷移に進む。バックグラウンドに回ってしまった処理が残っている場合は、その完了をツールで同期的に確認し切るか、同じ処理をフォアグラウンドで再実行してから次のフェーズへ進む
...
**この制約は `PreToolUse` フック（`scripts/block-async-execution.mjs`）でハード強制される**。以下のツール呼び出しは拒否（deny）されるため、拒否理由が返ってきたらエラーとして扱わず、フォアグラウンド同期実行に切り替えて続行すること：
...
拒否されたら、同じ処理を **フォアグラウンド（`run_in_background: false`）で再実行し、完了を待ってから次に進む**。E2Eテストのように時間がかかる処理も、待つのが正しい挙動であり、バックグラウンド化して待たずに進んではならない。
...
【実行モードの制約】
- Bash / Skill / Agent をバックグラウンド実行しない（`run_in_background: true`・コマンド末尾 `&`・`nohup` / `disown` / `setsid` の使用禁止）
- 時間のかかるテストも同期実行で完了を待つ。処理がタイムアウト等でバックグラウンドに回ってしまった場合は、その完了をツールで待ち切ってから報告する
- **バックグラウンドタスクが残った状態で完了報告してターンを終えない**（「完了次第報告します」「待機します」で終わらせない）。呼び出し元はあなたのターン終了を「作業完了」として扱う
...
- **サブエージェントの結果を鵜呑みにしない**: 完了報告は「やったつもり」を示すだけ。`git diff` で実際の差分を必ず検証する。「待機します」「完了次第報告します」のようにバックグラウンドタスクを残して返ってきた場合は完了ではない。`SendMessage` での再開はできないため、新しいフォアグラウンド `Agent` を起動して残作業を完遂させる
- **通知待ちでターンを終えない**: バックグラウンド化した処理やサブエージェントの「完了通知を待ちます」でターンを終えると、print モードではそのままプロセスが正常終了し、ワーカーがPR未作成のまま完了扱いでラベル遷移を進めてしまう。ターンを終えてよいのは、フェーズ7まで完遂した（またはフェーズ6でIssueをクローズした、または中断条件・フェーズ7のPR未作成手順で理由を出力した）場合のみ
