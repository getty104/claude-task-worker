...
- Playwright のブラウザ（chromium）が取得済み（`claude-task-worker install` / `update` が面倒を見る）
  ...
  - Linux では別途システムライブラリが必要（`sudo npx playwright-core@latest install-deps chromium`）。sudo を要求するため `install` / `update` は実行せず案内のみ出す
