...
- **デザイン先行フローへ移行しない例外は2つだけ**: (1) descriptionの `## UIデザイン` セクションに `.pen` の実パス行があり、どのデザインを参照して実装するかが明示されている場合（判定基準は `hasDesignReference()` と同一）、(2) 同セクションに `UIデザインは不要` で始まる行がある場合（`create-ui-design` の「デザイン不要」判定マーカー）。この場合のみそのままパターンEへ進む。「デザイン検討の余地が無い」「既存デザインの微修正」「リードタイムを伸ばしたくない」は例外の理由にならない（他に `.pen` の編集経路が無いため、例外はそのままデザイン未更新を意味する）
