...
# 1. ブランチ状況確認
git status
git log --oneline --graph "origin/${BASE_BRANCH}..HEAD"  # BASE_BRANCH は SKILL.md ステップ1の契約どおり導出する（デフォルトブランチ固定ではない）
...
git rebase -i "origin/${BASE_BRANCH}"  # BASE_BRANCH は SKILL.md ステップ1の契約どおり導出する（デフォルトブランチ固定ではない）
```
