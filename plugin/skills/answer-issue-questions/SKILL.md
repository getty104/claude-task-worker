...
description: "GitHub Issueの確認事項に対して、コードベース・ドキュメント・そこから参照されている外部リンク（仕様書・ライブラリ公式ドキュメント等）まで徹底的に調査し、根拠に基づいた回答を提供するスキル。Issueの最後のコメントとdescriptionの両方に含まれる確認事項を調査・回答し、コメントに追記する。調査しても事実で決まらず人間の意思決定が必要な項目は、固定セクションで明示して後続のトリアージへ引き渡す。"
...
GitHub Issueの確認事項（最後のコメント・descriptionの両方）を調査し、根拠に基づいた回答を提供するスキルです。

# Instructions

## 本スキルの位置づけ（確認事項の回答責任はここにある）

`triage-created-issue` は確認事項の調査を行わず、未回答の確認事項を見つけたら**内容を問わず本スキルへ委任する**。したがって「`gh` コマンドだけでは答えが決まらない項目」が回るのは正常であり、それを理由に回答を放棄して人へ差し戻してはならない。コードベース・ドキュメント・`.claude/requirements/`・外部リンク（仕様書・ライブラリ公式ドキュメント・関連Issue/PR）まで調査し、**事実で決まる項目は本スキルで決着させる**。

調査を尽くしてもなお事実で決まらず、人間の意思決定（優先度・ビジネス判断・不可逆な選択など）が必要な項目だけを、後述の固定セクション `## 人間判断が必要な確認事項（自動回答不能）` に列挙する。このセクションは `triage-created-issue` が `cc-need-human-check` を付与する唯一の根拠であり、セクションが無ければ「全項目を回答済み」として後続フロー（`cc-update-issue` → 再トリアージ → 着手）へ進む。**安易に列挙すると人の手が止まり、列挙を怠ると決まっていない判断が実装へ流れる**ため、判定基準（「人間判断が必要」と判定してよい条件）に厳密に従うこと。
...
- 方針の変更提案、代替案（A案/B案などの選択肢）の提示、「方針確定をお願いしたい」「いずれを採るか判断してほしい」といった**人間への意思決定の差し戻しを回答に含めない**
  - **例外は `## 人間判断が必要な確認事項（自動回答不能）` セクションだけ**。確認事項として明示的に問われている項目が、調査を尽くしても事実で決まらず Issue 本文の前提でも決まらない場合に限り、同セクションへ列挙してよい（判定基準は「人間判断が必要と判定してよい条件」を参照）。この例外は「問われた確認事項に答えられなかった」ことの申告であり、**問われていない方針を再審議する口実にはならない**。回答本文（`## 回答` の各項目）側には従来どおり方針の再審議・代替案の列挙を書かない
...
**確認事項は2箇所から収集する。両方を回答対象にする。**

1. **最後のコメント**: `## 確認事項` セクション、または箇条書きの問いかけ。従来からの主な入力源
2. **description（Issue本文）**: `## 確認事項` セクションのほか、「確認したいこと」「要確認」「TBD」「未確定」などの形で本文に残っている未確定項目。`triage-created-issue` はこれらも委任対象として本スキルを起動するため、コメント側に確認事項が無くても description 側だけで対象が成立しうる
...
ステップ2・3の分析結果をもとに、ステップ1で収集した確認事項の**全件**に回答を作成する。コメント由来・description由来を問わず、未回答の項目を残さない。

**投稿先**:

- 最後のコメントが確認事項コメントである場合は、そのコメントを編集して `## 回答` を追記する（既存挙動）
- description由来の確認事項しか無い場合、または最後のコメントが確認事項コメントでない場合は、`gh issue comment` で新規コメントとして `## 回答` を投稿する
- 両方に確認事項がある場合は、1つの `## 回答` にまとめて投稿・追記する（投稿口を複数に分けない。`update-issue` が拾う入力を一本化するため）。description由来の項目には見出しに `（description由来）` を添えて出所を明示する
...
- 自動回答不能として `## 人間判断が必要な確認事項（自動回答不能）` に列挙した項目の数と、その項目名（0件ならその旨。0件なら後続トリアージは `cc-need-human-check` を付けずに着手判定へ進む）
