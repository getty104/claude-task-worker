...
- `## デザインファイルの更新` — `triage-created-issue` のパターンE-1経路2が書き込んだ、実装PRで `.pen` も更新させる指示。消えるとUI変更だけが実装されデザインファイルが置き去りになる

配置は `## 影響範囲` の後・変更ログ折りたたみブロックの前とし、両方ある場合は既存bodyでの順序を保つ。`mode=create` では対象外（そもそも存在しない）。
...
- テンプレート外の見出し（`##`）を追加していない（末尾の変更ログ折りたたみは見出しではないため対象外。依存関係も `## 依存関係` として本文に書かず、`blocked_by` / `blocking`（GitHub relationships）で表現する）。ただし `mode=edit` で既存bodyにあった `## UIデザイン` / `## デザインファイルの更新` は上記の verbatim 保持ルールに従って残す
- `mode=edit` で既存bodyに `## UIデザイン` / `## デザインファイルの更新` があった場合、その中身が1文字も変わらずに再掲されている
