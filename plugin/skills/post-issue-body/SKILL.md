...
- `## 要件` / `## 実装プラン` が、PR の CI の結果（GitHub Actions の成否・CI が PR に投稿するコメント）を完了条件や作業の前提にしていない（例:「push して CI の出力を見てから値を書く」「CI の結果が差分なしであることを確認する」）。`exec-issue` は CI を待たずに PR を作って終えるため、こうした記述は実行されないか、CI 待ちでワーカーを張り付かせる。CI でしか得られない値に依存する場合は「ローカルで得られる情報から最終形を書き、CI の結果との差分は PR の修正ループ（`triage-pr` → `fix-review-point`）で寄せる」と書く
