...
呼び出し元の親スキル（`breakdown-issues`）は、**本スキル起動時の `args` に以下の YAML ブロックを文字列として渡す**こと。本スキルは受け取った入力を YAML として機械的にパースして扱う。
...
- args から入力 YAML を取得できない場合（直接ユーザー起動など）は、親スキル（`breakdown-issues`）の使用を促して中断する。
