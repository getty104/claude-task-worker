...
- `--cloud` は `CLOUD_DENIED_WORKERS` を**起動時エラーにはしない**。内訳は `resolve-conflict` / `create-ui-design` / `apply-ui-design` の3件のみで、いずれも `.pen` の編集に `pencil` CLI とその認証が要る／クラウドからの force-push 可否が未検証であることが理由。`all` / `yolo` には denied ワーカーが必ず含まれるため、エラー終了させると `--cloud` をこれらのコマンドで使えなくなる。そのため denied ワーカーは `isCloudWorker()` が false を返し、その分だけローカル実行のまま残す。どのワーカーがローカルに残るかは起動時に1行ログで示す
