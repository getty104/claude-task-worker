...
上記以外の観点（テストカバレッジ・Lint・型安全性など）はデザインPRには適用しない。CIが失敗している場合、失敗が `.pen`/スナップショットPNGの変更で解消可能な種類（デザインレビュー系のチェック等）であれば通常PRと同様に「対応すべき」とする。一方、ベースブランチ側の障害や実装コードに対する必須チェック（型チェック・ユニットテスト等、デザインPRの変更差分では触れられない失敗）が原因であれば、`.pen`変更のみで直せる見込みがなく`cc-fix-onetime`へ乗せても解消しないため「対応すべき」とはせず、ステップ3のパターンCへ進めて `cc-need-human-check` を付与する。
...
評価結果に基づき、以下のパターン（通常はA/Bのいずれか、デザインPRでCI失敗が解消不能な場合はC）で判定し、**必ずいずれかのアクションを実行**する。判定のみで終了せず、コマンドの実行まで確実に行う。
...
#### パターンC: デザインPRでCI失敗が`.pen`変更では解消不能な場合

`cc-ui-design` ラベル付きPRで、CI失敗の原因がベースブランチ側の障害や実装コードに対する必須チェック（`.pen`/スナップショットPNGの変更では直接修正できないもの）である場合、以下を実行して`cc-need-human-check`ラベルを追加し終了する。`cc-fix-onetime`は付与しない（コード修正でしか直らない失敗を`.pen`変更で解消しようとするtriage-pr⇔fix-review-pointの無限ループを避けるため）。
